#!/usr/bin/env python3
"""
ML Top4 + Mechanism v1 + v2 + v3_refined Features Training with GroupKFold CV
═════════════════════════════════════════════════════════════════════════════════
Data: final_features(2167) + v1(5) + v2(10) + v3_refined(4) merge
CV: GroupKFold(n_splits=5, groups=canonical_drug_id)
Models: Stacking_Ridge, CatBoost, RandomForest, XGBoost
Feature importance: Tree 3 models, v3_refined feature ranking
"""
import warnings
...
V3R_PATH = MECH_DIR / "mechanism_features_v3_refined_20260413.parquet"

N_FOLDS = 5
SEED = 42
OUTPUT_DIR = PROJECT_ROOT / "results" / "ml_mechanism_v2_v3refined_results_20260413"
...
def load_data():
    """Load final_features + v1 (295 drugs) + v2 (7730 rows) + v3_refined (7730 rows) merge."""
    print("=" * 70)
    print("Loading data...")
    t0 = time.time()
    ...
    # v3_refined: per-sample (7730 rows), merge on (sample_id, canonical_drug_id)
    df_v3r = pd.read_parquet(V3R_PATH)
    v3r_merge_cols = ["sample_id", "canonical_drug_id"] + V3R_FEATURE_COLS
    df_v3r = df_v3r[v3r_merge_cols]
    df = df.merge(df_v3r, on=["sample_id", "canonical_drug_id"], how="left")

    n_merged = len(df)

    # null 채우기
    for col in V1_FEATURE_COLS + V2_FEATURE_COLS + V3R_FEATURE_COLS:
    ...
    n_v3r = len([c for c in V3R_FEATURE_COLS if c in feat_cols])
    ...
    print(f"  v3_refined:     {len(df_v3r)} rows x {n_v3r} features")
    print(f"  Merged: {n_merged} rows (유지: {'OK' if n_final == n_merged else 'FAIL'})")
    print(f"  Total features: {n_total} "
          f"(final {n_final_feat} + v1 {n_v1} + v2 {n_v2} + v3r {n_v3r})")
...
def report_feature_importance(importances, feature_names, top_k=20):
    """Print top-K feature importance + v3_refined feature rankings."""
    print("\n" + "=" * 90)
    ...
    for model_name, imp in importances.items():
        ...
        # v3_refined feature positions
        print(f"\n  v3_refined Feature Rankings in {model_name}:")
        for v3r_col in V3R_FEATURE_COLS:
            if v3r_col in feature_names:
                v3r_idx = feature_names.index(v3r_col)
                v3r_rank = int((sorted_idx == v3r_idx).argmax()) + 1
                v3r_pct = imp_norm[v3r_idx] * 100
                print(f"    {v3r_col:<40s}  rank={v3r_rank:>4d} / {len(feature_names)}  "
                      f"({v3r_pct:.3f}%)")
...
def main():
    ...
    # ── Summary Table ──
    print("\n" + "=" * 90)
    print(f"  ML+MECHANISM v2+v3_refined TOP4 SUMMARY (GroupKFold, Total: {total_elapsed/60:.1f} min)")
    print("=" * 90)
    ...
    if v2_path.exists():
        ...
        print(f"\n{'─'*90}")
        print(f"  v2 MECHANISM vs v2+v3_refined MECHANISM (Spearman)")
        print(f"{'─'*90}")
        print(f"{'Model':<22} {'v2 Sp':>8} {'v2+v3r Sp':>10} {'Delta':>7}  "
              f"{'Train Sp':>10} {'Gap':>7}")
        print("-" * 90)
        ...
        for r in all_results:
            ...
            if v2_name in v2_dict:
                ...
                print(f"{r['model']:<22} {bl['spearman_mean']:>8.4f} "
                      ...
                      f"{r['train_spearman_mean']:>10.4f} "
                      f"{r['gap_spearman_mean']:>7.4f}")

        print("-" * 90)

    # ── v2+v3 raw vs v2+v3_refined 비교 ──
    v2v3_path = PROJECT_ROOT / "results" / "ml_mechanism_v2_v3_results_20260413" / "ml_mechanism_v2_v3_results.json"
    if v2v3_path.exists():
        with open(v2v3_path) as f:
            v2v3_data = json.load(f)
        v2v3_dict = {r["model"]: r for r in v2v3_data["model_results"]}

        print(f"\n{'─'*90}")
        print(f"  v2+v3_raw vs v2+v3_refined (Spearman)")
        print(f"{'─'*90}")
        print(f"{'Model':<22} {'v2+v3_raw':>10} {'v2+v3_refined':>14} {'Delta':>7}")
        print("-" * 90)

        for r in all_results:
            if r["model"] in v2v3_dict:
                raw = v2v3_dict[r["model"]]
                sp_delta = r["spearman_mean"] - raw["spearman_mean"]
                sp_arrow = "+" if sp_delta > 0 else ""
                print(f"{r['model']:<22} {raw['spearman_mean']:>10.4f} "
                      f"{r['spearman_mean']:>14.4f} {sp_arrow}{sp_delta:>6.4f}")
    ...
    for model_name, imp in importances.items():
        if imp is not None:
            ...
            if total > 0:
                ...
                v3r_ranks = {}
                for v3r_col in V3R_FEATURE_COLS:
                    if v3r_col in feat_names:
                        v3r_idx = feat_names.index(v3r_col)
                        v3r_rank = int((sorted_idx == v3r_idx).argmax()) + 1
                        v3r_ranks[v3r_col] = {
                            "rank": v3r_rank,
                            "importance": float(imp_norm[v3r_idx]),
                        }
                fi_report[model_name] = {
                    "top20": top20,
                    "v3_refined_feature_ranks": v3r_ranks,
                }
    ...
    results_path = OUTPUT_DIR / "ml_mechanism_v2_v3refined_results.json"
