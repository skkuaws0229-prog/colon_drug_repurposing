...
## biso_myprotocol · say2-4team · v2.0
**작성일:** 2026-04-08 | **수정일:** 2026-04-10 | **작성자:** say2-4team | **버전:** v2.0

> **v2.0 변경사항 (20260410):**
> - Step 4: MultiModalFusionNet 추가 (5-모달 Cross-Attention)
> - Step 5: 앙상블 v2 — 6개 → 7개 모델 (MultiModalFusionNet 포함)
> - Step 8: 멀티모달 허들 게이트 신규 추가 (H3~H6)
> - 전체 소요시간 업데이트
...
## STEP 8. 멀티모달 허들 게이트 [20260410 신규]
**예상 소요 시간: 10~20분**

> Step 7 ADMET 통과 약물(Top 15)을 멀티모달 관점에서 추가 검증하는 단계.
> 기존 문헌 기반 분류와 93% 일치 확인 (biso 기준).

### 8-1. 허들 구조
```
H1: IC50 예측 (MultiModalFusionNet)
    High (<1.0) / Medium (1.0~3.0) / Low (>3.0)
H2A: Lipinski Rule of Five (IV 약물 완화 적용)
H2B: 22-assay ADMET 정밀 검증
     safety_score: ≥6 PASS / 4~6 WARNING / <4 FAIL
H3: Tanimoto 유사도
    FDA 승인 유방암 치료제 11개와 비교
    >0.3 PASS / 0.1~0.3 WARNING / <0.1 FAIL
H4: 유방암 핵심 타겟 필터 (14개)
    BRCA1, BRCA2, HER2, ESR1, PGR,
    PIK3CA, AKT1, PTEN, CDK4, CDK6,
    CCND1, TP53, MYC, EGFR
    (+A 후보: PALB2, ATM, FGFR1/2, RAD51, RB1)
H5: AlphaFold 도킹 (stub → 향후 구현)
H6: 내성 변이 경고 (stub → 향후 구현)
최종 판정: PASS / WARNING / FAIL
```

### 8-2. 실행
```bash
cd 20260410_multimodal_fusion/models/
python run_step8_hurdles_20260410.py \
  --input  admet_results/top15_drugs.csv \
  --output hurdle_results/step8_results_20260410.csv

python inference.py --smiles "SMILES입력"
```

### 8-3. biso 기준 결과 (유방암 Top 15)
```
전체 일치율: 14/15 (93%)
약물 재창출 후보:
- Sepantronium  ADMET 10/12 ★
- Camptothecin  ADMET 10/12 ★
- Staurosporine ADMET 10/12 ★
```

### QC 체크
```
[ ] H1~H4 허들 실행 완료
[ ] IV 약물 Lipinski 완화 적용 확인
[ ] 기존 분류 일치율 확인 (≥90%)
[ ] 약물 재창출 후보 확정
[ ] 결과 S3 저장 완료
```
