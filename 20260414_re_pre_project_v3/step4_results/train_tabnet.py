...
print("="*80)
print("Model 11/15: TabNet (GPU-2, 간소화 MLP)")
print("="*80)

device = "mps" if torch.backends.mps.is_available() else "cpu"
print(f"Device: {device}")

# TabNet 대신 간소화된 MLP 사용
...
model_final = SimpleMLP(X_train.shape[1]).to(device)
...
with open("model_11_tabnet.json", "w") as f:
    json.dump({"model": "TabNet (SimpleMLP)", "device": device, "oof_spearman": float(oof_sp), "oof_rmse": float(oof_rmse),
               "holdout_spearman": float(holdout_sp), "holdout_rmse": float(holdout_rmse),
               "ensemble_pass": bool(ensemble_pass)}, f, indent=2)
print("✓ TabNet (SimpleMLP) 완료")
