#!/usr/bin/env python3
"""
Step 4 Drug Structure Train Spearman 계산
과적합 분석 (GraphSAGE 포함 7개)
"""
import os
import json
import numpy as np
...
print("="*80)
print("Step 4 Drug Structure Train Spearman 계산")
print("="*80)
...
# Recreate drug-based split (same seed)
np.random.seed(42)
...
# Model configurations (7 models including GraphSAGE)
models_info = [
    (1, 'LightGBM', 'cpu', 'pkl'),
    (2, 'DART', 'cpu', 'pkl'),
    (4, 'CatBoost', 'cpu', 'pkl'),
...
class GraphSAGEMLP(torch.nn.Module):
    def __init__(self, input_dim):
        super().__init__()
        self.agg1 = torch.nn.Sequential(
            torch.nn.Linear(input_dim, 512),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.2)
        )
        self.agg2 = torch.nn.Sequential(
...
for model_id, model_name, model_type, ext in models_info:
    ...
    with open(f"step4_scaffold_{model_id:02d}_results.json") as f:
        result_data = json.load(f)
    test_sp = result_data['metrics']['spearman']

    # Load model and predict on training data
    model_file = f"step4_scaffold_{model_id:02d}_model.{ext}"
    ...
    if model_type == 'cpu':
        ...
        if model_id == 10:
            ...
            model = GraphSAGEMLP(X.shape[1]).to(device)
    ...
    with open(f"step4_scaffold_{model_id:02d}_results.json", 'w') as f:
...
with open('step4_scaffold_train_spearman_summary.json', 'w') as f:
    json.dump(summary, f, indent=2)

print("\n" + "="*80)
print("Step 4 Train Spearman 계산 완료!")
print("="*80)
