#!/usr/bin/env python3
"""
2단계: GroupKFold GPU 모델
대상: FlatMLP, CrossAttention, FT-Transformer
"""
import os
import json
import numpy as np
...
def train_groupkfold_gpu(model_name, model_id, model_class):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups=drug_ids), 1):
        ...
        print(f"  Sp: {metrics['spearman']:.4f}, RMSE: {metrics['rmse']:.4f}")
...
result_flatmlp = train_groupkfold_gpu("FlatMLP", 10, FlatMLP)
result_crossattn = train_groupkfold_gpu("CrossAttention", 13, CrossAttentionMLP)
result_fttrans = train_groupkfold_gpu("FT-Transformer", 12, FTTransformerMLP)
