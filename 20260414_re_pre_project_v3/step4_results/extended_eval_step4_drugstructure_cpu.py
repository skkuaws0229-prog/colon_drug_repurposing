#!/usr/bin/env python3
"""
Step 4: Drug Structure Split (CPU 모델)
약물 구조 기반 분할 (scaffold proxy)
"""
import os
import json
import numpy as np
...
print("="*80)
print("Step 4: Drug Structure Split (CPU)")
print("="*80)
...
# Drug-based split (similar to scaffold split)
# 20% drugs as test set
np.random.seed(42)
...
print(f"\nData Split (Drug-Structure Proxy):")
...
def train_drugstruct_cpu(model_name, model_id, model_params):
    print(f"\n{'='*80}")
...
for model_name, model_id, params in models_config:
    train_drugstruct_cpu(model_name, model_id, params)
