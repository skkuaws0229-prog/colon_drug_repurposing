#!/usr/bin/env python3
"""
Model 14/15: GraphSAGE (GPU, Drug-Split)
Protocol v3.0 - Simplified GraphSAGE with drug-based splitting
"""
import os
...
print("="*80)
print("Model 14/15: GraphSAGE (GPU, Drug-Split)")
print("="*80)
...
drug_ids = features_slim['canonical_drug_id'].values  # Extract drug column

# Simplified GraphSAGE-inspired MLP (without actual graph structure)
# This uses aggregation concept but on flattened features
class GraphSAGEMLP(nn.Module):
    def __init__(self, input_dim):
        super().__init__()
        # Feature aggregation layers (mimicking neighbor aggregation)
...
for fold_idx, (drug_tr_idx, drug_val_idx) in enumerate(kf.split(unique_cv_drugs), 1):
    ...
    # Model
    model = GraphSAGEMLP(X_tr.shape[1]).to(device)
...
final_model = GraphSAGEMLP(X_cv.shape[1]).to(device)
...
with open("model_14_graphsage.json", "w") as f:
    json.dump(results, f, indent=2)

np.save("model_14_graphsage_oof.npy", oof_predictions)
np.save("model_14_graphsage_holdout.npy", holdout_pred)

print("✓ GraphSAGE (Drug-Split) 완료")
