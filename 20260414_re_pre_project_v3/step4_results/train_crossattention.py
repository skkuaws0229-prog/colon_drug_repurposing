#!/usr/bin/env python3
"""
Model 13/15: CrossAttention (GPU-2)
Protocol v3.0 - Multi-head cross-attention MLP
"""
import os
...
print("="*80)
print("Model 13/15: CrossAttention (GPU-2)")
print("="*80)
...
class CrossAttentionMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # MLP
        return self.mlp(attn_output).squeeze()
...
for fold_idx, (tr_idx, val_idx) in enumerate(kf.split(X_cv), 1):
    ...
    # Model
    model = CrossAttentionMLP(X_tr.shape[1]).to(device)
...
final_model = CrossAttentionMLP(X_cv.shape[1]).to(device)
...
with open("model_13_crossattention.json", "w") as f:
    json.dump(results, f, indent=2)

np.save("model_13_crossattention_oof.npy", oof_predictions)
np.save("model_13_crossattention_holdout.npy", holdout_pred)

print("✓ CrossAttention 완료")
