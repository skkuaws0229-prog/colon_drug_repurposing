#!/usr/bin/env python3
"""
Model 8/15: RSF (Random Survival Forest adapted for regression)
Protocol v3.0 - Using RandomForest as proxy
"""
import os
...
print("="*80)
print("Model 8/15: RSF (Random Survival Forest)")
print("="*80)
print("Note: Using RandomForestRegressor as RSF proxy for IC50 regression")
...
for fold_idx, (tr_idx, val_idx) in enumerate(kf.split(X_cv), 1):
    ...
    # RSF-like configuration (deeper trees, more focus on individual samples)
    model = RandomForestRegressor(
        n_estimators=300,
        max_depth=None,
        min_samples_split=5,  # Slightly more conservative
        min_samples_leaf=2,   # Require at least 2 samples per leaf
        max_features='sqrt',  # Like survival forests
        n_jobs=-1,
...
with open("model_08_rsf.json", "w") as f:
    json.dump(results, f, indent=2)

np.save("model_08_rsf_oof.npy", oof_predictions)
np.save("model_08_rsf_holdout.npy", holdout_pred)
