...
def _load_drug_name_map():
    """Load DRUG_ID -> DRUG_NAME map. Fallback to empty map if unavailable."""
    try:
        ann = pd.read_parquet(DRUG_ANN_URI)
        if "DRUG_ID" not in ann.columns or "DRUG_NAME" not in ann.columns:
            return {}
        m = ann[["DRUG_ID", "DRUG_NAME"]].dropna()
        m["DRUG_ID"] = m["DRUG_ID"].astype(str).str.strip()
        m["DRUG_NAME"] = m["DRUG_NAME"].astype(str).str.strip()
        m = m[m["DRUG_ID"] != ""]
        return dict(zip(m["DRUG_ID"], m["DRUG_NAME"]))
    except Exception as e:
        print(f"  [WARNING] Could not load drug annotation map: {e}")
        return {}
...
def main():
    ...
    # ── Direct competition across all drugs → Top 30/Top 15 ──
    print(f"\n{'='*60}")
    print(f"  Direct Drug Competition & Top 15 Selection")
    print(f"{'='*60}")
    ...
    drug_summary["drug_id"] = drug_summary["drug_id"].astype(str).str.strip()

    # Add drug names for name-level deduplication
    drug_name_map = _load_drug_name_map()
    drug_summary["drug_name"] = drug_summary["drug_id"].map(
        lambda x: drug_name_map.get(x, f"Drug_{x}")
    )
    drug_summary["drug_name_key"] = drug_summary["drug_name"].apply(_normalize_drug_name)
    ...
    drug_summary["category"] = drug_summary["sensitivity_rate"].apply(
        lambda x: "Validated" if x > 0.5 else "Recommended"
    )

    # Direct competition score over ALL available drugs (not pre-trimmed Top30)
    drug_summary["score"] = (
        -drug_summary["mean_pred_ic50"].rank()  # lower IC50 = better
        + drug_summary["sensitivity_rate"].rank() * 2  # higher sensitivity = better
        + (drug_summary["n_samples"] >= 5).astype(int) * 5  # bonus for sufficient samples
    )

    # De-duplicate by normalized drug name (keep highest score)
    comp = drug_summary.sort_values(
        ["score", "mean_pred_ic50", "sensitivity_rate"],
        ascending=[False, True, False],
    ).copy()
    total_ids = len(comp)
    comp_unique = comp.drop_duplicates(subset=["drug_name_key"], keep="first").copy()
    dropped_dups = total_ids - len(comp_unique)

    # Competition ranks (after dedup)
    comp_unique["competition_rank"] = range(1, len(comp_unique) + 1)
    top30 = comp_unique.head(30).copy()
    top15 = comp_unique.head(15).copy()
    top15 = top15.sort_values("mean_pred_ic50", ascending=True).copy()
    top15["final_rank"] = range(1, len(top15) + 1)

    print(f"\n  Direct competition pool: {total_ids} IDs")
    print(f"  Unique drug names after dedup: {len(comp_unique)} (dropped {dropped_dups} duplicate IDs)")

    print(f"\n  Top 30 Drugs (direct competition):")
    print(f"  {'Rank':<5} {'Drug':<22} {'Drug ID':<12} {'Pred IC50':>10} {'True IC50':>10} "
          f"{'Sens Rate':>10} {'N':>4} {'Category':<12}")
    print(f"  {'-'*100}")
    for _, row in top30.iterrows():
        print(f"  {int(row['competition_rank']):<5} {row['drug_name'][:22]:<22} {row['drug_id']:<12} "
              f"{row['mean_pred_ic50']:>10.3f} {row['mean_true_ic50']:>10.3f} "
              f"{row['sensitivity_rate']:>10.1%} {int(row['n_samples']):>4} {row['category']:<12}")

    print(f"\n  {'='*80}")
    print(f"  TOP 15 SELECTED DRUGS")
    print(f"  {'='*80}")
    print(f"  {'#':<4} {'Drug':<22} {'Drug ID':<12} {'Pred IC50':>10} {'True IC50':>10} "
          f"{'Sens Rate':>10} {'N':>4} {'Category':<12}")
    print(f"  {'-'*100}")
    for _, row in top15.iterrows():
        print(f"  {int(row['final_rank']):<4} {row['drug_name'][:22]:<22} {row['drug_id']:<12} {row['mean_pred_ic50']:>10.3f} "
    ...
    results = {
        ...
        "competition_pool": {
            "total_drug_ids": int(total_ids),
            "unique_drug_names": int(len(comp_unique)),
            "dedup_dropped_ids": int(dropped_dups),
            "scoring_mode": "direct_competition_all_drugs",
        },
        "top30_drugs": top30[["competition_rank", "drug_id", "drug_name", "mean_pred_ic50", "mean_true_ic50",
                              "sensitivity_rate", "n_samples", "category", "score"]].to_dict(orient="records"),
        "top15_drugs": top15[["final_rank", "competition_rank", "drug_id", "drug_name", "mean_pred_ic50", "mean_true_ic50",
                              "sensitivity_rate", "n_samples", "category", "score"]].to_dict(orient="records"),
    }
